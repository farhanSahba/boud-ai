<aside
    class="lqd-adv-img-editor-sidebar group/sidebar max-md:group-[&.active]/editor:motion-preset-slide-bottom-md fixed bottom-0 end-0 grid w-[--sidebar-w] grid-cols-1 place-items-start bg-background shadow-[0_4px_44px_rgba(0,0,0,0.06)] transition-all motion-duration-[0.35s] md:group-[&.active]/editor:motion-preset-slide-left-md group-[&.active]/editor:motion-opacity-in-[0%] max-md:max-h-[calc(100vh-30px-(var(--header-h,0px)*2))] max-md:overflow-y-auto max-md:pt-8 max-md:group-[&.sidebar-collapsed]/editor:translate-y-[calc(100%-115px)] max-md:group-[&.sidebar-collapsed]/editor:overflow-hidden md:top-0 md:pt-[--header-h] md:group-[&.sidebar-collapsed]/editor:translate-x-[calc(100%-40px)]"
    x-ref="imageEditorSidebar"
>
    <x-button
        class="max-md:group-[&.active]/editor:motion-preset-slide-bottom-md absolute top-5 z-10 size-[38px] -translate-x-1/2 bg-background motion-duration-[0.35s] motion-delay-200 md:group-[&.active]/editor:motion-preset-slide-left-md max-md:left-1/2 max-md:h-1 max-md:w-24 max-md:rounded-full max-md:border-none max-md:bg-foreground/10 max-md:before:absolute max-md:before:left-1/2 max-md:before:top-1/2 max-md:before:h-16 max-md:before:w-full max-md:before:-translate-x-1/2 max-md:before:-translate-y-1/2 md:start-0 md:top-24"
        variant="outline"
        size="none"
        hover-variant="primary"
        @click.prevent="switchSidebarCollapsed()"
        ::class="{ 'md:rotate-180': sidebarCollapsed }"
    >
        <x-tabler-chevron-right class="size-4 max-md:hidden" />
    </x-button>

    <form
        class="lqd-adv-img-editor-sidebar-inner col-start-1 col-end-1 row-start-1 row-end-1 h-full w-full px-5 pb-9 pt-7 transition-all duration-300 motion-duration-[0.45s] motion-delay-150 md:group-[&.active]/editor:motion-preset-slide-left-sm group-[&.sidebar-collapsed]/editor:opacity-0 max-md:pb-24 max-md:group-[&.sidebar-collapsed]/editor:translate-y-2 md:overflow-y-auto md:px-7 md:group-[&.sidebar-collapsed]/editor:translate-x-2"
        :class="{ 'md:scale-95 md:opacity-0 md:invisible md:pointer-events-none': typeof creativeSuite !== 'undefined' && creativeSuite?.stageInitiated }"
    ...
    >
        ...
        <div class="flex min-h-full grow flex-col gap-y-7">
            <h3
                class="m-0 empty:hidden max-md:group-[&.sidebar-collapsed]/editor:hidden md:hidden"
                x-text="tools?.find(t => t.action === selectedTool)?.title ?? ''"
            ></h3>

            <div
                class="group/dropdown relative max-md:hidden"
                x-data='{
                    dropdownOpen: false,
                }'
            ...
            <template
                x-if="(! ['remove_text', 'cleanup', 'remove_background', 'upscale', 'merge_face'].includes(selectedTool)) || ['nano-banana/edit', 'nano-banana-pro/edit', 'nano-banana-2/edit', 'xai/grok-imagine-image/edit'].includes(aiModel)"
            >
                <x-forms.input
