...
@push('script')
	<script>
		...
		document.addEventListener('livewire:init', function() {
			Livewire.on('mediaSelected', function(eventData) {
				...
				if (window.currentFileInput && data && data.items) {
						...
						if (loadedCount === totalCount) {
							setTimeout(() => {
								...
								if (hasVideoFiles) {
									...
									setGenerateButtonState(false, 'Video Loaded âœ“');
									setTimeout(() => {
										setGenerateButtonState(false, 'Generate');
									}, 1500);
								}
					...
					Promise.all(
						items.map(async (item, index) => {
							try {
								...
								console.error(`â‌Œ Failed to load file from URL: ${item.url}`, error);
					...
					console.warn('âڑ ï¸ڈ Media selection event missing required data:', {
		...
		function sendOpenaiGeneratorForm(ev) {
			...
			document.getElementById("openai_generator_button").innerHTML = bued_localize.please_wait;
