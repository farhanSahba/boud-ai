...
class Helper
{
    ...
    {
        return self::appIsDemo() && in_array(request()?->getHost(), ['bued-ai.com', 'www.bued-ai.com']);
    }
    ...
    {
        $now = new DateTimeImmutable; // إ‍u anki tarih
        $startDate = $now->modify('-1 month'); // 1 ay أ¶ncesi
        $interval = new DateInterval('P1D'); // Gأ¼nlأ¼k adؤ±m
    ...
    public static function demoDataForAdminDashboardPopularPlans(): array
    {
        return [
            ...
            [
                'label' => 'Monthly',
                'value' => 280,
                'color' => '#FFC107', // Amber / Sarؤ±msؤ±
            ],
            [
                'label' => 'Prepaid',
                'value' => 120,
                'color' => '#F44336', // Kؤ±rmؤ±zؤ±
            ],
        ];
    }
    ...
    public static function checkDemoSecondDailyLimit($incomingSeconds = 0, $lockKey = 'default_second_lock_key'): JsonResponse
    {
        ...
        // Release the lock (no update yet â€” we only update after successful generation)
        Cache::lock($lockKey)->release();
