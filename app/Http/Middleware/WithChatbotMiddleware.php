...
trait WithChatbotMiddleware
{
    ...
    {
        $appKey = $fromInput
            ? $this->request()->input('appKey')
            : ($this->request()->headers->get('X-Bued-AI-Chatbot') ?? $this->request()->headers->get('X-MagicAI-Chatbot'));
    ...
    public static function corsHeaders(string $originUrl = '*'): array
    {
        return [
            ...
            'Access-Control-Allow-Headers'     => 'Accept, Origin, Content-Type, X-Bued-AI-Chatbot, X-MagicAI-Chatbot, X-Livewire, Authorization',
