...
trait InstallTheme
{
    public function install(string $extensionSlug): bool|array
    {
        ...
        $folderName = data_get($responseExtension, 'extension_folder', $extensionSlug);

        if ($extensionSlug === 'default' || $folderName === 'default') {
            setting([
                'front_theme' => 'default',
                'dash_theme'  => 'default',
            ])->save();
        ...
        if (! file_exists(resource_path("views/$folderName/theme.json"))) {
            return [
                'status'  => false,
                'message' => trans('Theme files were not found locally'),
            ];
        }

        $version = data_get($responseExtension, 'version', '1.0.0');
        $themeType = data_get($responseExtension, 'theme_type', 'All');
