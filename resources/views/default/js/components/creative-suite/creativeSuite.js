	...
	return ({
		...
		createNode(options = { type: null, attrs: {} }) {
			...
			const attrs = {
				...
				text: options.attrs?.text || bued_localize.double_click_to_edit,
				fontSize: 30,
		...
		resetCanvas() {
			if (
				this.nodes.length &&
				confirm(bued_localize['all_your_current_edits_will_be_lost__are_you_sure_'])
			) {
				this.resetHistory();
				this.destroyNodes({ nodes: 'all' });
				this.fitToScreen();
		...
		beforeUnloadHandler(event) {
			if (this.nodes.length > 0) {
				const message = bued_localize['you_have_unsaved_changes__are_you_sure_you_want_to_leave_'];
		...
		async loadTemplatesList(url) {
			...
			if ( !(data instanceof Array) || !data.length ) {
				...
				toastr.error(bued_localize['could_not_fetch_templates_list_please_try_again'] || 'Could not fetch templates list. Please try again.');
				return;
			}
		...
		loadTemplate(templateId) {
			...
			if ( !templateData ) {
				return toastr.error(bued_localize['could_not_fetch_template_data_please_try_again'] || 'Could not fetch template data. Please try again.');
			}
		...
		async saveDocument() {
			...
			try {
				...
				if (responseData.status === 'error') {
					toastr.error(responseData.message || bued_localize['error_saving_document']);
					return;
				}
				...
				toastr.success(bued_localize['saved_succesfully']);
			} catch (error) {
		...
		async duplicateDocument(docId) {
			...
			try {
				...
				if (responseData.data) {
					...
					toastr.success(bued_localize['duplicated_the_document_successfully']);
				}
			} catch (error) {
				toastr.error('Upload failed:', error);
			}
		...
		async deleteDocument(docId) {
			...
			try {
				...
				if (responseData.status === 'success') {
					...
					toastr.success(bued_localize['document_deleted_successfully']);
				}
			} catch (error) {
				toastr.error('Upload failed:', error);
			}
		...
		async updateDocumentName(event) {
			...
			try {
				...
				toastr.success(bued_localize['name_updated_successfully']);
			} catch (error) {
