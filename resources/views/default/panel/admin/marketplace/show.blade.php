                ...
                >
                    ...
                    @else
                        <x-button
                            ...
                            size="lg"
                            target="_blank"
                            href="{!! $item['routes']['redirect'] ? $item['routes']['redirect'] . '&email=' . auth()->user()->email() : '#' !!} "
                        >
                            {{ __('Live Preview') }}
                        </x-button>
                    @endif
                ...
                @if (!\App\Helpers\Classes\Helper::isUserVIP() && !$marketSubscription['data'] && $item['price'] != 0 && \App\Helpers\Classes\Helper::appIsNotDemo())
                    ...
                    >
                        ...
                        <p class="mb-6 font-medium">
                            {{ trans('Premium membership unlocks full access to all Bued features and exclusive benefits. Stand out from the crowd at an affordable monthly rate.') }}
                        </p>
...
@push('script')
    ...
    <script>
        document.getElementById('copyButton')?.addEventListener('click', function() {
            navigator.clipboard.writeText('{{ config('mail.from.address', 'admin@example.com') }}');
