    ...
    >
        ...
        @else
            @if ($gatewayDefinition['live_client_id'] == 1)
                <x-forms.input
                    ...
                    size="lg"
                    label="{{ $gatewayDefinition['code'] === 'tap' ? __('Publishable Key') : __('Api Key / Client Id') }}"
                    required
                />
            @endif
            @if ($gatewayDefinition['live_client_secret'] == 1)
        ...
        @if ($gatewayDefinition['sandbox_client_id'] == 1)
            <x-forms.input
                ...
                size="lg"
                label="{{ ($gatewayDefinition['code'] === 'tap' ? __('Publishable Key') : __('Api Key / Client Id')) . ' ' }} ({{ __('Sandbox') }})"
            />
        @endif
