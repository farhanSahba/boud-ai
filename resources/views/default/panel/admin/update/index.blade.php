		...
		<div class="container-xl">
			<div class="col-12 md:px-10">
				@if ($app_is_not_demo)
					...
					<div class="card mb-2 shadow-sm">
						<div class="card-body p-6">
							<div class="flex items-center justify-between flex-wrap gap-4">
								<div>
									<h5 class="text-heading font-semibold mb-2">{{ __('Update Channel') }}</h5>
									<p class="text-sm text-gray-600 dark:text-gray-400 mb-0">
                                        <span id="channel_description">
                                            @if(env('BUED_AI_USER_TYPE', env('MAGICAI_USER_TYPE', '')) === 'tester')
												{{ __('You are currently on the Beta channel. You will receive early access to new features and updates.') }}
											@else
												{{ __('You are currently on the Stable channel. You will receive thoroughly tested updates.') }}
											@endif
                                        </span>
									</p>
								</div>
								<div class="flex items-center gap-3">
									<div class="flex items-center gap-2">
										<span class="text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Stable') }}</span>
										<label class="relative inline-flex items-center cursor-pointer">
											<input
												type="checkbox"
												id="beta_toggle"
												class="sr-only peer"
												@if(env('BUED_AI_USER_TYPE', env('MAGICAI_USER_TYPE', '')) === 'tester') checked @endif
											>
											<div class="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-purple-300 dark:peer-focus:ring-purple-800 rounded-full peer dark:bg-gray-700 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:border-gray-600 peer-checked:bg-purple-600"></div>
										</label>
										<span class="text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Beta') }}</span>
									</div>
								</div>
							</div>
						</div>
					...
					<!-- Beta Warning Alert -->
					<div
						class="card mb-6 bg-amber-50 border-l-4 border-amber-400 dark:bg-amber-900/20"
						id="beta_warning"
						style="display: {{ env('BUED_AI_USER_TYPE', env('MAGICAI_USER_TYPE', '')) === 'tester' ? 'block' : 'none' }};"
					>
						<div class="card-body p-6">
							<div class="flex gap-4">
								...
								<div>
									<h5 class="font-semibold text-amber-800 dark:text-amber-200 mb-2">{{ __('Beta Channel Active') }}</h5>
					...
					>
						...
						<div class="card-body p-10">
							<div class="row">
								<div class="col-12">
									<h4 class="text-heading mb-6 text-[23px] leading-[1.3em]">{{ __('A newer version of') }} {{ $setting->site_name }} {{__('is available! أ¢إ“آ¨')}}</h4>
...
@push('script')
	<script>
		$(document).ready(function() {
			...
			// Beta toggle functionality
			$('#beta_toggle').on('change', function() {
				const isBeta = $(this).is(':checked');
				const channelName = isBeta ? 'Beta' : 'Stable';
				const warningText = isBeta
					? 'أ¢ع‘آ أ¯آ¸عˆ You are switching to the Beta channel. Beta updates may be less stable and should only be used in testing environments. Always backup before updating.\n\nAre you sure you want to continue?'
					: 'You are switching to the Stable channel. You will only receive thoroughly tested updates.\n\nDo you want to continue?';

				if (confirm(warningText)) {
					toggleBetaChannel(isBeta);
				} else {
					// Revert toggle if user cancels
					$(this).prop('checked', !isBeta);
				}
			});
		});

		function toggleBetaChannel(enableBeta) {
			// Disable toggle during request
			$('#beta_toggle').prop('disabled', true);

			$.ajax({
				type: 'POST',
				url: '{{ route('dashboard.update.channel.convert') }}',
				data: {
					_token: '{{ csrf_token() }}',
					enable_beta: enableBeta
				},
				success: function(response) {
					if (response.success) {
						toastr.success(response.message || 'Update channel changed successfully. Reloading page...');
						...
						toastr.error(response.message || 'Failed to change update channel');
						// Revert toggle on error
						$('#beta_toggle').prop('checked', !enableBeta).prop('disabled', false);
					}
				},
				error: function(xhr) {
					const errorMessage = xhr.responseJSON?.message || 'An error occurred while changing the update channel';
			...
			$.ajax({
				...
				success: function(response) {
					...
					<small>Current Version: ${response.current_version}</small>`
						...
						localStorage.setItem("bued_ai_update_badge", badgeHtml);
						$('#update_available').show();
						$('#update_card').show();
						$('#update_not_available').hide();
					} else {
						$('#update_not_available').show();
						$('#update_card').hide();
						localStorage.removeItem("bued_ai_update_badge");
					}
					...
					if (response.description) {
						...
						$('#update_description').html(
							'<a href="https://bued-ai.com" target="_blank">Visit Changelog on the Documentation Page</a>'
						);
					}
				}
			});
