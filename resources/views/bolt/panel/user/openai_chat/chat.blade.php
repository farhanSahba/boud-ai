...
@section('titlebar_actions')
    @include('components.select-ai-model-list')

    <x-button href="{{ route('dashboard.user.openai.list') }}">
...
@section('content')
    ...
    <div class="py-0">
        <div
            class="chats-wrap group/chats-wrap relative h-[calc(100vh-7rem)] md:grid md:h-[75vh] md:grid-flow-col md:[grid-template-columns:30%_70%] lg:[grid-template-columns:25%_75%]"
            id="user_chat_area"
            :class="{ 'chats-sidebar-hidden': $store.focusMode.active && sidebarHidden }"
            x-data="{ mobileOptionsShow: false, toggleMobileOptions() { this.mobileOptionsShow = !this.mobileOptionsShow }, mobileSidebarShow: false, toggleMobileSidebar() { this.mobileSidebarShow = !this.mobileSidebarShow }, sidebarHidden: false }"
        >
            <div
                class="chats-sidebar-wrap relative flex w-full transition-all max-md:absolute max-md:start-0 max-md:top-20 max-md:h-0 max-md:overflow-hidden lg:fixed lg:bottom-[--body-padding] lg:end-[--body-padding] lg:top-[--body-padding] lg:z-5 lg:!w-[--sidebar-width] lg:overflow-y-auto lg:rounded-e-2xl lg:border-s lg:border-border lg:bg-background [&.active]:h-[calc(100%-80px)]"
    ...
    @includeIf('panel.user.openai_chat.includes.share-modal-include')
    ...
    <template id="chat_ai_bubble">
        <div
            class="lqd-chat-ai-bubble group mb-2 flex content-start gap-2"
            data-message-id=""
            data-title=""
        >
            <span
            ...
            <div
                ...
                <div class="inline-flex max-w-full items-center rounded-full font-medium leading-none">
                    <div
                        class="chat-content prose relative w-full max-w-none px-6 py-3 indent-0 font-[inherit] text-xs font-normal text-current [word-break:break-word] empty:hidden [&_*]:text-current">
                    </div>
                    <div
