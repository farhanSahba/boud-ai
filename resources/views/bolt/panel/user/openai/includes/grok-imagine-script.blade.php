<script>
    function sendGrokImagineGeneratorForm(ev) {
        ev?.preventDefault();
        ev?.stopPropagation();

        const submitBtn = document.getElementById("grok_imagine_generator_button");

        document.getElementById("grok_imagine_generator_button").disabled = true;
        document.getElementById("grok_imagine_generator_button").innerHTML = magicai_localize.please_wait;
        ...
        formData.append('description', $("#description_grok_imagine").val());
        formData.append('model', 'xai/grok-imagine-image');
        formData.append('image_number_of_images', 1);
        formData.append('image_generator', 'xai/grok-imagine-image');
        ...
        formData.append('description_grok_imagine', $('#description_grok_imagine').val());
        ...
        $.ajax({
            ...
            success: function(res) {
                ...
                if (res.status !== 'success' && (res.message)) {
                    document.getElementById("grok_imagine_generator_button").disabled = false;
                    document.getElementById("grok_imagine_generator_button").innerHTML = "Regenerate";
                ...
                setTimeout(function() {
                    @if ($openai->type == 'image')
                        ...
                        images.forEach((image) => {
                            ...
                            if (image.request_id || image.status === 'IN_QUEUE') {
