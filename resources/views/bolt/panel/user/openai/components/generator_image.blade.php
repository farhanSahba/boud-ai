...
<div
    class="lqd-titlebar-secondary -mx-4 mb-20"
    x-data="{
...
>
    ...
    >
        <h1 class="m-0 text-center text-black">
            <svg
                class="me-1.5 inline-block align-text-top"
                width="20"
                height="20"
                viewBox="0 0 20 20"
                fill="currentColor"
            ...
            >
                <path
                    d="M11.7342 4.655L0.749987 15.6408C0.298669 16.1248 0.0528549 16.765 0.0643506 17.4266C0.0758463 18.0883 0.343754 18.7196 0.811611 19.1876C1.27947 19.6555 1.91073 19.9236 2.57235 19.9352C3.23398 19.9469 3.87428 19.7012 4.35832 19.25L15.345 8.26583L11.7342 4.655ZM12.9883 8.26583L10.8333 10.4208L9.58332 9.16667L11.7383 7.01167L12.9883 8.26583ZM3.17999 18.0733C3.01153 18.2339 2.78772 18.3235 2.55499 18.3235C2.32225 18.3235 2.09845 18.2339 1.92999 18.0733C1.7645 17.9074 1.67157 17.6827 1.67157 17.4483C1.67157 17.214 1.7645 16.9892 1.92999 16.8233L8.40415 10.3483L9.65832 11.6025L3.17999 18.0733ZM17.7775 12.19L20 13.3017L17.7775 14.4167L16.6667 16.635L15.5558 14.4167L13.3333 13.3017L15.5558 12.19L16.6667 9.96833L17.7775 12.19ZM5.55582 4.44417L3.33332 3.33333L5.55582 2.2225L6.66665 0L7.77749 2.2225L9.99999 3.33333L7.77749 4.44417L6.66665 6.66667L5.55582 4.44417ZM16.1108 3.88917L14.1667 2.91667L16.1108 1.94417L17.0833 0L18.0558 1.94417L20 2.91667L18.0558 3.88917L17.0833 5.83333L16.1108 3.88917Z"
                />
            </svg>
            @lang('Explain your idea.')
            <span
                class="cursor-pointer opacity-50 transition-colors hover:opacity-100"
                @click="prompt = generateRandomPrompt()"
            >
                @lang('Generate example prompt.')
            </span>

            @if (setting('user_ai_image_prompt_library') == null || setting('user_ai_image_prompt_library'))
                <button
                    class="lqd-generator-templates-trigger inline-flex size-10 shrink-0 cursor-pointer items-center justify-center gap-2 rounded-full align-middle text-inherit transition-all max-md:h-auto max-md:w-auto max-md:bg-transparent md:hover:bg-heading-background md:hover:text-heading-foreground"
                    type="button"
                    @click.prevent="togglePromptLibraryShow()"
                >
                    <span class="sr-only">{{ __('Browse prompt library') }}</span>
                    <x-tabler-article
                        class="size-6"
                        stroke-width="1.5"
                    />
    ...
    <div class="container relative z-2 -mt-14 flex flex-col">
        <div
            class="mx-auto flex w-full max-w-[650px] items-center gap-5 rounded-2xl bg-background/90 px-4 py-6 shadow-[0_4px_10px_hsl(0_0%_0%/4%)] dark:shadow-[0_4px_20px_rgba(255,255,255,0.05)]">
            <x-card
                class:body="flex flex-col gap-5"
                class="lqd-image-generator"
                variant="none"
                size="none"
            >
                <div class="flex flex-wrap justify-between">
                    <div class="lqd-image-generator-tabs-nav flex gap-1">
                        @if (setting('dalle_hidden', 0) != 1)
                            <x-button
                                class="lqd-image-generator-tabs-trigger active"
                        ...
                        @if (setting('enabled_gpt_image_1', '0') != '0')
                            <x-button
                                class="lqd-image-generator-tabs-trigger"
                ...
                @if (setting('dalle_hidden', 0) != 1)
                    ...
                    >
                        ...
                        >
                            <div class="lqd-image-generator-inputs-wrap relative mb-2 w-full">
                                @foreach (json_decode($openai->questions) ?? [] as $question)
                                    @if ($question->type == 'textarea')
                                        <x-forms.input
                                            class="lqd-image-generator-prompt h-[53px] resize-none overflow-hidden rounded-2xl border-[3px] bg-background px-4 py-3.5 text-heading-foreground placeholder:text-foreground focus:ring-0 max-md:min-h-32 max-md:rounded-md"
                                            id="{{ $question->name }}"
                                            type="textarea"
                                            name="{{ $question->name }}"
                                            x-data="{}"
                                ...
                                <x-button
                                    class="absolute end-4 top-1/2 -translate-y-1/2 hover:-translate-y-1/2 hover:scale-110 max-lg:relative max-lg:right-auto max-lg:top-auto max-lg:mt-2 max-lg:w-full max-lg:translate-y-0 lg:size-[30px] lg:p-0"
                            ...
                            <x-button
                                class="lqd-generator-advanced-trigger group text-3xs font-semibold text-heading-foreground lg:hidden"
                            ...
                            <div
                                class="active flex w-full flex-wrap justify-between gap-3 max-lg:hidden lg:fixed lg:bottom-[--body-padding] lg:end-[--body-padding] lg:top-[--body-padding] lg:z-5 lg:w-[--sidebar-width] lg:flex-col lg:flex-nowrap lg:justify-start lg:gap-8 lg:overflow-y-auto lg:rounded-e-2xl lg:border-s lg:border-border lg:bg-background lg:p-7 max-lg:[&.advanced-opts-enabled]:flex lg:[&.inactive]:hidden lg:[&>div]:grow-0"
                                x-data="{}"
                                style="view-transition-name: lqd-sidebar"
                                :class="{ 'inactive': activeGenerator !== 'openai', 'active': activeGenerator === 'openai', 'advanced-opts-enabled': advancedSettingsShow }"
                            >
                                <h3 class="hidden border-b border-border pb-6 text-start text-lg lg:block">
                                    @lang('Advanced Settings')
                                </h3>
                                @include('panel.user.openai.components.generator_image_dalle_options')
                                <div class="sticky -bottom-7 -mx-7 mt-auto hidden rounded-xl px-7 py-4 backdrop-blur-2xl lg:block">
                                    <x-button
                                        class="w-full"
                                        type="submit"
                                    >
                ...
                @if (setting('enabled_gpt_image_1', '0') != '0')
                    ...
                    >
                        ...
                        >
                            <h3
                                class="mb-0 flex w-full flex-wrap items-center gap-2"
                            ...
                            <div
                                ...
                                x-data="{}"
                                style="view-transition-name: lqd-sidebar"
                                :class="{ 'inactive': activeGenerator !== 'gpt_image_1', 'active': activeGenerator === 'gpt_image_1', 'advanced-opts-enabled': advancedSettingsShow }"
                            >
                                <h3 class="hidden border-b border-border pb-6 text-start text-lg lg:block">
                                    @lang('Advanced Settings')
                                </h3>
                    ...
                    >
                        ...
                        >
                            ...
                            <div
                                ...
                                x-data="{}"
                                style="view-transition-name: lqd-sidebar"
                                :class="{ 'inactive': activeGenerator !== 'gpt_image_1_5', 'active': activeGenerator === 'gpt_image_1_5', 'advanced-opts-enabled': advancedSettingsShow }"
                            >
                                <h3 class="hidden border-b border-border pb-6 text-start text-lg lg:block">
                                    @lang('Advanced Settings')
                                </h3>
                ...
                >
                    <form
                        class="lqd-image-generator-stablediffusion-form flex flex-col gap-5"
                        id="openai_generator_form"
                        onsubmit="return sendOpenaiGeneratorForm();"
                    >
                        <div class="lqd-tabs-nav flex flex-wrap">
                            <x-button
                                class="lqd-tabs-trigger active grow rounded-e-none bg-heading-foreground/5 text-heading-foreground hover:translate-y-0 hover:bg-primary hover:text-primary-foreground [&.active]:bg-primary [&.active]:text-primary-foreground"
                                variant="ghost"
                                tag="button"
                            ...
                            <x-button
                                class="lqd-tabs-trigger grow rounded-none bg-heading-foreground/5 text-heading-foreground hover:translate-y-0 hover:bg-primary hover:text-primary-foreground [&.active]:bg-primary [&.active]:text-primary-foreground"
                                variant="ghost"
                                tag="button"
                            ...
                            <x-button
                                class="lqd-tabs-trigger grow rounded-s-none bg-heading-foreground/5 text-heading-foreground hover:translate-y-0 hover:bg-primary hover:text-primary-foreground [&.active]:bg-primary [&.active]:text-primary-foreground"
                                variant="ghost"
                                tag="button"
                        ...
                        <div
                            class="lqd-tabs-content flex-col items-start"
                        ...
                        >
                            <div class="lqd-image-generator-inputs-wrap relative w-full">
                                @foreach (json_decode($openai->questions) ?? [] as $question)
                                    @if ($question->type == 'textarea')
                                        <x-forms.input
                                            class="h-[53px] resize-none overflow-hidden rounded-2xl border-[3px] bg-background px-4 py-3.5 text-heading-foreground placeholder:text-foreground focus:ring-0 max-md:min-h-32 max-md:rounded-md"
                        ...
                        <div
                            class="lqd-tabs-content hidden flex-col items-start gap-4"
                        ...
                        >
                            <x-forms.input
                                class="multi_prompts_description h-[53px] resize-none overflow-hidden rounded-2xl border-[3px] bg-background px-4 py-3.5 text-heading-foreground placeholder:text-foreground focus:ring-0 max-md:min-h-32 max-md:rounded-md"
                        ...
                        >
                            ...
                            <div
                                class="inactive flex w-full flex-wrap justify-between gap-3 max-lg:hidden lg:fixed lg:bottom-[--body-padding] lg:end-[--body-padding] lg:top-[--body-padding] lg:z-5 lg:w-[--sidebar-width] lg:flex-col lg:flex-nowrap lg:justify-start lg:gap-8 lg:overflow-y-auto lg:rounded-e-2xl lg:border-s lg:border-border lg:bg-background lg:p-7 max-lg:[&.advanced-opts-enabled]:flex lg:[&.inactive]:hidden lg:[&_.lqd-input-group]:m-0 lg:[&_.lqd-input-group]:grow-0 lg:[&_.lqd-input-group]:flex-col lg:[&_.lqd-input-group]:flex-nowrap lg:[&_.lqd-input-group]:gap-8"
                                x-data="{}"
                                style="view-transition-name: lqd-sidebar"
                                :class="{ 'inactive': activeGenerator !== 'stable_diffusion', 'active': activeGenerator === 'stable_diffusion', 'advanced-opts-enabled': advancedSettingsShow }"
                            >
                                <h3 class="hidden border-b border-border pb-6 text-start text-lg lg:block">
                                    @lang('Advanced Settings')
                                </h3>
                ...
                <div
                    ...
                    x-data="{}"
                    style="view-transition-name: lqd-sidebar"
                    :class="{ 'inactive': activeGenerator !== 'flux-pro' && activeGenerator !== 'ideogram', 'active': activeGenerator === 'flux-pro' || activeGenerator === 'ideogram' }"
                >
                </div>
...
@if (Route::has('dashboard.user.photo-studio.index') && setting('photo_studio', 1) == 1)
    ...
    >
        <div class="lqd-img-gen-tools-trigger-wrap flex items-center gap-8">
            <button
                class="lqd-img-gen-tools-trigger active flex w-full items-center gap-2.5 text-[21px]/[1.25em] font-medium text-heading-foreground"
            ...
            >
                ...
                <x-tabler-chevron-down
                    class="ms-auto size-5 rotate-180 transition-transform"
        ...
        <div
            class="lqd-img-gen-tools grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-3"
            :class="{ hidden: !toolsShow, grid: toolsShow }"
        >
            @foreach ($ai_tools as $tool)
                <x-card
                    class:body="p-10"
                    class="lqd-img-gen-tool relative overflow-hidden bg-card-background before:absolute before:inset-0 before:z-0 before:bg-current before:opacity-50 before:transition-all hover:-translate-y-1 hover:shadow-lg hover:shadow-black/5 dark:before:opacity-5"
                    size="none"
                    variant="none"
                    roundness="sm"
                    style="color: {{ $tool['color'] }}"
                >
                    <h4 class="lqd-img-gen-tool-title relative z-1 m-0 flex items-center gap-3.5 text-lg font-medium">
                        <span
                            class="lqd-img-gen-tool-icon inline-flex size-6 shrink-0 items-center justify-center rounded-lg transition-all [&_svg]:h-auto [&_svg]:w-full"
                            style="color: {{ $tool['color'] }}"
                        >
                            {!! $tool['icon'] !!}
                        </span>
...
>
    <h2 class="mb-5 text-[21px]">
        {{ __('Result') }}
    </h2>
    <div class="image-results grid grid-cols-2 gap-8 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6">
