...
@section('title', __('🌁 AI Image Generator'))

@section('titlebar_before')
    <div class="flex w-full grow items-center justify-center">
        <div class="max-sm:w-full md:min-w-72">
            <x-credit-list
                x-init="generator = '{{ setting('dalle_hidden', 0) == 1 ? 'stable_diffusion' : 'openai' }}'"
                :aiImage="true"
                :showLegend="true"
            />
        </div>
...
<x-card
    class="lqd-image-generator mb-10 border-0 bg-surface-background shadow-md shadow-black/[3%]"
    class:body="p-8"
    size="none"
    x-data="{
        ...
        >
            ...
            >
                ...
                <div class="lqd-image-generator-inputs-wrap relative w-full">
                    ...
                    @endforeach
                    <x-button
                        class="size-14 shrink-0 rounded-3xl bg-gradient-to-r from-gradient-from via-gradient-via to-gradient-to hover:scale-110 max-lg:relative max-lg:right-auto max-lg:top-auto max-lg:mt-2 max-lg:w-full max-lg:translate-y-0"
        ...
        >
            ...
            >
                <div class="lqd-image-generator-inputs-wrap relative w-full">
                    ...
                    <x-button
                        class="size-14 shrink-0 rounded-3xl bg-gradient-to-r from-gradient-from via-gradient-via to-gradient-to hover:-translate-y-1/2 hover:scale-110 max-lg:relative max-lg:right-auto max-lg:top-auto max-lg:mt-2 max-lg:w-full max-lg:translate-y-0"
                        id="openai_generator_button"
                        tag="button"
                        type="submit"
                    >
            ...
            >
                ...
                <x-button
                    class="szie-[54px] w-full rounded-xl bg-gradient-to-r from-gradient-from via-gradient-via to-gradient-to"
                    id="openai_generator_button"
                    tag="button"
                    type="submit"
                    size="lg"
            ...
            >
                ...
                <x-button
                    class="szie-[54px] mt-2 w-full rounded-xl bg-gradient-to-r from-gradient-from via-gradient-via to-gradient-to"
                    id="openai_generator_button"
                    tag="button"
                    type="submit"
                    size="lg"
...
@if (Route::has('dashboard.user.photo-studio.index') && setting('photo_studio', 1) == 1)
    ...
    >
        <div class="lqd-img-gen-tools-trigger-wrap flex items-center gap-8">
            ...
            <button
                class="lqd-img-gen-tools-trigger active flex shrink-0 items-center gap-2.5 text-sm font-bold"
        ...
        >
            @foreach ($ai_tools as $tool)
                <x-card
                    class="lqd-img-gen-tool rounded-[20px] bg-card-background hover:-translate-y-1 hover:border-surface-background hover:bg-surface-background hover:shadow-lg hover:shadow-black/5 [&_svg:not([fill=none])]:fill-current [&_svg:not([stroke=none])]:stroke-current"
                    class:body="py-10 px-4"
                    size="none"
                    variant="outline"
                    roundness="none"
                >
                    <div class="flex items-center justify-center gap-4">
                        <div class="lqd-img-gen-tool-icon inline-flex shrink-0 [&_svg]:size-7">
                            {!! $tool['icon'] !!}
                        </div>
                        <div>
                            <h4 class="lqd-img-gen-tool-title mb-0 text-[18px] font-bold">
                                {{ __($tool['title']) }}
                            </h4>
                        </div>
                    </div>
