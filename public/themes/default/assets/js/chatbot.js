jQuery(document).ready(function($) {
	...
	$(document).on('click', '#bued-ai-chatbot-widget .bued-ai-chatbot-widget--trigger', function (e) {
		console.log(magicai_js_options)
		if ( !magicai_js_options.guest_status ) {
			toastr.error(bued_localize?.please_activate_bued_ai ||
						'Please activate Bued AI');
			return false;
		}

		var $this = $(this);
		var chat = $this.parent().find('.bued-ai-chatbot-widget--chat');
		var messages = chat.find('.bued-ai-chatbot-widget--messages');
		var message_list = chat.find('.bued-ai-chatbot-widget--message-list');
		chat.toggleClass('active');


		if ( ! $this.hasClass('loaded') ){
			$('#bued-ai-chatbot-widget').addClass('is-loading');
			messages.empty();
			jQuery.post( magicai_js_options.ajaxurl, { action: 'magicai_chatbot_get_chat' }, function ( response ) {
				if ( response.chat_history ) {
					$('#bued-ai-chatbot-widget .bued-ai-chatbot-widget--chat').toggleClass('active-message-list');
					message_list.append(response.output);
				} else {
					$('#bued-ai-chatbot-widget form').attr('data-id', response.chat_id);
					messages.append(response.output);
				}
				$('#bued-ai-chatbot-widget').removeClass('is-loading');
	...
	$(document).on('click', '#bued-ai-chatbot-widget .start-new-chat', function (e) {
		...
		var messages = $('#bued-ai-chatbot-widget .bued-ai-chatbot-widget--messages');
		messages.empty();
		var message_list = $('#bued-ai-chatbot-widget .bued-ai-chatbot-widget--message-list');

		$('#bued-ai-chatbot-widget .bued-ai-chatbot-widget--chat').removeClass('active-message-list');
		$('#bued-ai-chatbot-widget').addClass('is-loading');
	...
	$(document).on('click', '#bued-ai-chatbot-widget .message-list', function (e) {
	...
	$(document).on('click', '#bued-ai-chatbot-widget .bued-ai-chatbot-widget--message-list--item', function (e) {
		...
		$('#bued-ai-chatbot-widget').addClass('is-loading');
		$('#bued-ai-chatbot-widget .bued-ai-chatbot-widget--chat').toggleClass('active-message-list');
		$('#bued-ai-chatbot-widget form').attr('data-id', chat_id);
	...
	$(document).on('submit', '#bued-ai-chatbot-widget form', function (e) {
		...
		let result = $('#bued-ai-chatbot-widget .bued-ai-chatbot-widget--messages');
		...
		result.append(`<div class="bued-ai-chatbot-widget--message">${prompt}</div>`);
		...
		jQuery.post( magicai_js_options.ajaxurl, { action: 'magicai_chatbot_get_chat_data', chat_id: chat_id }, function ( response ) {
			if ( response.error ){
				...
				result.append(`<div class="bued-ai-chatbot-widget--message ai text-${message_id}">${response.message}</div>`);
				result.scrollTop(result[0].scrollHeight - result[0].clientHeight);
				return false;
			} else {
				result.append(`<div class="bued-ai-chatbot-widget--message ai text-${message_id}"></div>`);
				result.scrollTop(result[0].scrollHeight - result[0].clientHeight);
	...
	let asyncChatBot = async (prompt,result,message_id,message_data, chat_id) => {
		...
		let message = result.find( '.bued-ai-chatbot-widget--message.text-' + message_id );
		...
		jQuery.post( magicai_js_options.ajaxurl, { action: 'magicai_chatbot_save_chat_data', prompt: prompt_first, message: chunk.join(''), chat_id: chat_id }, function ( response ) {
			result.scrollTop(result[0].scrollHeight - result[0].clientHeight);
			$('#bued-ai-chatbot-widget form').removeClass('is-working');
		});
	};
