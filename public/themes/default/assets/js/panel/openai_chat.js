...
async function handleCanvasResponseStore(responseObj) {
	try {
		...
		if (!res.ok || resData.status == 'error') {
			toastr.error(resData.message || bued_localize.could_not_save_the_canvas_data);
		}
	} catch (error) {
		alert(error);
		console.error(error);
...
function getFrontModelEl() {
	...
	if (!chatbotFrontModel) {
		...
		defaultOption.value = '';
		defaultOption.textContent = bued_localize.default_model || 'Default Model';
...
function sendRequest(type, images, responseObj, sharedMessageUUID = null) {
	...
	fetchEventSource('/dashboard/user/generator/generate-stream', {
		...
		onerror: err => {
			...
			responseObj.responseStreaming = false;
			responseObj.response.push(`${bued_localize.error}: ${err.message}`);
...
async function startGenerateRequest(ev) {
	...
	if (!promptInputValue.trim()) {
		return toastr.error(bued_localize?.please_fill_message || 'Please fill the message field',);
	}
...
function startNewDocChat(file, type) {
	...
	$.ajax({
		...
		success: function (data) {
			...
			toastr.success(bued_localize.analyze_file_finish);
		},
		error: function (data) {
			Alpine.store('appLoadingIndicator').hide();
			$('.lqd-upload-doc-trigger').attr('disabled', false);
...
$(document).ready(function () {
		...
		if (confirm(`Are you sure you want to remove ${chatTitle}?`)) {
			...
			$.ajax({
				...
				success: function (data) {
					...
					toastr.success(
						bued_localize.conversation_deleted_successfully,
					);
				},
				error: function (data) {
					var err = data.responseJSON.errors;
	...
	$('#chat_sidebar_container').on('click', '.chat-item-pin', ev => {
		...
		$.ajax({
			...
			success: function (data) {
				toastr.success(isPinned ? bued_localize.conversation_unpinned : bued_localize.conversation_pinned);
			},
			error: (xhr, status, error) => {
				console.error('Error updating pin status:', error);
				togglePinMode();
				toastr.error(bued_localize.conversation_pin_error);
			},
		});
	});
	...
	$('#selectDocInput').change(function () {
		if (this.files && this.files[0]) {
			...
			toastr.success(bued_localize.analyze_file_begin);
