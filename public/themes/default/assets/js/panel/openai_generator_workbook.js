...
const generate = async ( message_no, creativity, maximum_length, number_of_results, prompt, openai_id, open_router_model) => {
	...
	if (stream_type == 'backend') {
		...
		try {
			...
			switch (error.status) {
				case 429:
					toastr.error(bued_localize?.api_connection_error || 'Api Connection Error. You hit the rate limites of openai requests. Please check your Openai API Key');
					break;
				default:
					toastr.error(bued_localize?.api_connection_error_admin || 'Api Connection Error. Please contact system administrator via Support Ticket. Error is: API Connection failed due to API keys');
			}
...
function getTinyMCEOptions(el) {
	...
	let toolbar = 'styles | buedIconRewrite | buedAIButton | link | image | forecolor backcolor emoticons | bold italic underline | bullist numlist | blockquote | wordcount | alignleft aligncenter alignright | code supercode';

	if ( isGeneratorV2 ) {
		toolbar += ' | buedAIEmoji buedAICover';
	}
	...
	return {
		...
		setup: function ( editor ) {
			...
			const menuItems = {
				'customwrite': {
					icon: 'magicIcon',
					text: bued_localize?.what_would_you_like_to_do || 'What would you like to do?',
					onAction: function ( e ) {
				...
				'rewrite': {
					icon: 'buedIconRewrite',
					text: bued_localize?.rewrite || 'Rewrite',
					onAction: function () {
						if (editor.selection.getContent().trim().length == 0) {
							toastr.warning('Please select text');
							return;
				...
				'summarize': {
					...
					text: bued_localize?.summarize || 'Summarize',
					onAction: function () {
						if (editor.selection.getContent().trim().length == 0) {
							toastr.warning('Please select text');
							return;
				...
				'makeitlonger': {
					...
					text: bued_localize?.make_it_longer || 'Make it Longer',
					onAction: function () {
						if (editor.selection.getContent().trim().length == 0) {
							toastr.warning('Please select text');
							return;
				...
				'makeitshorter': {
					...
					text: bued_localize?.make_it_shorter || 'Make it Shorter',
					onAction: function () {
						if (editor.selection.getContent().trim().length == 0) {
							toastr.warning('Please select text');
							return;
				...
				'improvewriting': {
					...
					text: bued_localize?.improve_writing || 'Improve Writing',
					onAction: function () {
						if (editor.selection.getContent().trim().length == 0) {
							toastr.warning('Please select text');
							return;
				...
				'translateto': {
					...
					text: bued_localize?.translate_to || 'Translate to',
					getSubmenuItems: function () {
						...
						items.push({
							...
							text: bued_localize?.search || 'Search',
				...
				'changestyle': {
					...
					text: bued_localize?.change_style_to || 'Change Style To',
					getSubmenuItems: function () {
						const styles = [
							'Professional',
							'Conversational',
				...
				'changetone': {
					...
					text: bued_localize?.change_tone_to || 'Change Tone To',
					getSubmenuItems: function () {
						const tones = [
							'Formal',
							'Informal',
				...
				'simplify': {
					...
					text: bued_localize?.simplify || 'Simplify',
					onAction: function () {
						if (editor.selection.getContent().trim().length == 0) {
							toastr.warning('Please select text');
							return;
				...
				'fixgrammaticalmistakes': {
					...
					text: bued_localize?.fix_grammatical_mistakes || 'Fix grammatical mistakes',
					onAction: function () {
						if (editor.selection.getContent().trim().length == 0) {
							toastr.warning('Please select text');
							return;
			...
			editor.ui.registry.addButton( 'buedIconRewrite', {
				icon: 'buedIconRewrite',
				text: bued_localize?.rewrite || 'Rewrite',
				onAction: menuItems.rewrite.onAction
			} );

			editor.ui.registry.addMenuButton( 'buedAIButton', {
				icon: 'magicIcon',
				fetch: callback => {
...
`);
				...
				if ( emojiPickerWrapper ) {
					...
					editor.ui.registry.addButton( 'buedAIEmoji', {
						text: bued_localize?.add_emjoi ?? 'Add Emoji',
						icon: 'magicIconEmoji',
						onAction: function() {
				...
				editor.ui.registry.addButton( 'buedAICover', {
					text: bued_localize?.add_cover ?? 'Add Cover',
					icon: 'magicIconCover',
					onAction: function() {
