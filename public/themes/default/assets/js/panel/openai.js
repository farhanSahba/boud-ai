...
function updateStatus( status, entry_id ) {
	...
	$.ajax( {
		...
		success: function ( data ) {
			...
			toastr.success(bued_localize?.status_changed ||'Status changed succesfully');
			templateRow?.classList?.toggle('active', status == 1);
			templateRow?.classList?.toggle('passive', status == 0);
		},
		error: function ( data ) {
			toastr.error(bued_localize?.something_wrong ||'Something went wrong. Please reload the page and try it again');
		}
	} );
	return false;
}
...
function templateSave(template_id) {
	...
	document.getElementById('custom_template_button').innerHTML = bued_localize.please_wait;
	...
	$.ajax({
		...
		processData: false,
		success: function(data) {
			toastr.success(bued_localize?.template_saved ||'Template Saved Successfully');
...
$( document ).ready( function () {
	...
	$( '.add-more' ).click( function () {
		...
		if ( anInputIsEmpty ) {
			return toastr.error(bued_localize?.fill_all_fields ||'Please fill all fields in User Group Input areas');
		}
	...
	$( 'body' ).on( 'click', '.remove-inputs-group', function () {
		...
		if ( currentInputGroupts.length <= 1 ) {
			return toastr.error(bued_localize?.at_least_one_group ||'You must have at least one group of inputs');
		}
