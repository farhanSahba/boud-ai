function sendSupportForm() {
	...
	$.ajax( {
		...
		success: function ( data ) {
			toastr.success(bued_localize?.support_ticket_created ||'Support Ticket Created Succesfully. Redirecting...');
			setTimeout( function () {
...
function sendMessage( ticket_id ) {
	...
	$.ajax( {
		...
		processData: false,
		success: function ( data ) {
			toastr.success(bued_localize?.message_sent ||'Message sent succesfully. Please Wait');
			setTimeout( function () {
				location.reload();
			}, 1500 );
		},
