...
function LoginForm() {
	...
	document.getElementById('LoginFormButton').innerHTML = bued_localize.please_wait;
	Alpine.store('appLoadingIndicator').show();

	var email = $('#email').val();
	if (email == '') {
		toastr.error(bued_localize.missing_email);
		document.getElementById('LoginFormButton').disabled = false;
		document.getElementById('LoginFormButton').innerHTML = bued_localize.sign_in;
		Alpine.store('appLoadingIndicator').hide();
		return false;
	}
	var password = $('#password').val();
	if (password == '') {
		toastr.error(bued_localize.missing_password);
		document.getElementById('LoginFormButton').disabled = false;
		document.getElementById('LoginFormButton').innerHTML = bued_localize.sign_in;
		Alpine.store('appLoadingIndicator').hide();
		return false;
	...
	$.ajax({
		...
		success: function (data) {
			toastr.success(bued_localize.login_redirect);
...
function RegisterForm() {
	...
	btn.disabled = true;
	btn.innerHTML = bued_localize.please_wait;
	Alpine.store('appLoadingIndicator').show();
	...
	$.ajax({
		...
		processData: false,
		success: function (data) {
			toastr.success(bued_localize.register_redirect);
			setTimeout(function () {
				if (plan) {
		...
		error: function (data) {
			...
			if (type === 'confirmation') {
				...
				btn.disabled = false;
				btn.innerHTML = bued_localize.signup;
				Alpine.store('appLoadingIndicator').hide();
			}
		}
	});
...
function PasswordResetMailForm() {
	...
	document.getElementById('PasswordResetFormButton').innerHTML = bued_localize.please_wait;
	Alpine.store('appLoadingIndicator').show();
	...
	$.ajax({
		...
		processData: false,
		success: function (data) {
			toastr.success(bued_localize.password_reset_link);
			Alpine.store('appLoadingIndicator').hide();
		},
		error: function (data) {
			var err = data.responseJSON.errors;
...
function PasswordReset(password_reset_code) {
	...
	$.ajax({
		...
		processData: false,
		success: function (data) {
			toastr.success(bued_localize.password_reset_done);
			setTimeout(function () {
		...
		error: function (data) {
			...
			document.getElementById('PasswordResetFormButton').disabled = false;
			document.getElementById('PasswordResetFormButton').innerHTML = bued_localize.password_reset;
			Alpine.store('appLoadingIndicator').hide();
		}
	});
	return false;
