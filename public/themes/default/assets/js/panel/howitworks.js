function howitWorksSave( howitWorks_id ) {
	...
	document.getElementById( "item_edit_button" ).innerHTML = bued_localize.please_wait;
	...
	$.ajax( {
		...
		success: function ( data ) {
			toastr.success(bued_localize?.how_it_works_step_saved ||'How it Works Step Saved Succesfully. Redirecting...')
			setTimeout( function () {
...
function howitWorksUpdate(from) {
	...
	$.ajax( {
		...
		processData: false,
		success: function ( data ) {
			toastr.success(bued_localize?.how_it_works_bottom_line_saved ||'How it Works Bottom Line updated successfully. Redirecting...')
			setTimeout( function () {
				location.href = '/dashboard/admin/howitWorks';
			}, 1000 );
		},
