...
function sendRequestForm() {
	...
	document.getElementById( "send_request_button" ).innerHTML = bued_localize.please_wait;
	...
	$.ajax( {
		...
		processData: false,
		success: function ( data ) {
			toastr.success(bued_localize?.request_sent ||'Request Sent Succesfully');
			setTimeout( function () {
				location.reload();
			}, 900 );
		},
		error: function ( data ) {
			toastr.error(bued_localize?.you_cannot_withdrawal ||'You cannot withdrawal with this amount. Please check')
...
function sendInvitationForm() {
    ...
    document.getElementById( "send_invitation_button" ).innerHTML = bued_localize.please_wait;
    ...
    $.ajax( {
        ...
        processData: false,
        success: function ( data ) {
            toastr.success(bued_localize?.invitation_sent ||'Invitation Sent Succesfully!');
        ...
        error: function ( data ) {
            toastr.error(bued_localize?.error_while_sending ||'Error while sending information. Please contact us.')
            document.getElementById( "send_invitation_button" ).disabled = false;
            document.getElementById( "send_invitation_button" ).innerHTML = "Send Invitation";
        }
    } );
