...
function editWorkbook(workbook_slug) {
	...
	$.ajax({
		...
		success: function (data) {
			toastr.success(bued_localize?.workbook_saved ||'Workbook Saved Succesfully');
		},
		error: function (data) {
			toastr.error(bued_localize?.workbook_error ||'Workbook  Error');
		},
		complete: function () {
...
document.addEventListener('DOMContentLoaded', async function () {
	...
	const tinymceOptions = {
		...
		toolbar: 'styles | buedIconRewrite | buedAIButton | image | link | forecolor backcolor emoticons | bold italic underline  | bullist numlist | alignleft aligncenter alignright | code supercode',
	...
	$('body').on('click', '#workbook_copy', () => {
		if (window.codeRaw) {
			...
			toastr.success(bued_localize?.code_copied ||'Code copied to clipboard');
			return;
		}
		if (tinymce?.activeEditor) {
			...
			toastr.success(bued_localize?.content_copied ||'Content copied to clipboard');
			return;
		}
	});
