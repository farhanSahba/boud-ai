$(document).ready(function () {
	...
	$('[data-toogle="cart"]').on('click', function (event) {
		...
		toastr.info(bued_localize.updating_cart || 'Updating Cart');
	...
	$('.btn_install').on('click', function () {
		...
		$.ajax({
			...
			processData: false,
			success: function (data) {
				toastr.success(bued_localize?.addon_installed ||'Add-on installed succesfully.');
				setTimeout(() => {
	...
	$('.btn_installed').on('click', function () {
		...
		$.ajax({
			...
			processData: false,
			success: function (data) {
				toastr.success(bued_localize?.addon_uninstalled ||'Add-on uninstalled succesfully.');
				setTimeout(() => {
					window.location.reload();
				}, 2000);
			},
