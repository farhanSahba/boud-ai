(() => {
	...
	function copyToClipboard(content, contentType = 'text') {
		...
		navigator.clipboard.writeText(content).then(() => {
			toastr.success(
				bued_localize && bued_localize[`${contentType}_content_copied_to_clipboard`] ?
					bued_localize[`${contentType}_content_copied_to_clipboard`] :
					'Content copied to clipboard' );
		}).catch(err => {
			toastr.error( bued_localize?.copy_failed || 'Failed to copy content' );
		});
	}
